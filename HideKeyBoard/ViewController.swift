       ...
       DismissKeyboard()
        ...
        
        {
           // Call MessageBox if any fields are empty
            MsgBox("All fields required, please correc")
        }
        else
        {
            ...
            if (txtContacts.text=="")
            {
                //if empty ten add text and newline
                txtContacts.text = "MyContacts \n"
            }
            //format text
            //existing contents of txtContacts, newine, txtFirst, newline, txtLast, newline, txtEmail, newline
            ...
            //load scrollview
            //declare CGPoint scrollPoint
            var scrollPoint:CGPoint//x = from Left, Y = from Top
            //get x = 0, Y = location fo btnBack Y
            scrollPoint = CGPointMake (0, btnBack.frame.origin.y)
